...
let serviceAccount = require("./website-87a77-firebase-adminsdk-3yndw-ab11a98d57.json");
admin.initializeApp({
...
const bucketName = "ecommerce-website-shawn-glen";
const accessKeyId = 'AKIAY7I4NSCGXLEVB7WA'
const secretAccessKey = '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'
aws.config.update({
    region,
    accessKeyId,
    secretAccessKey
...
app.post('/order', (req, res) => {
    ...
    let transporter = nodemailer.createTransport({
        ...
        secure: false,
        auth: {
            user: 'lordarglax@gmail.com',
            pass: 'fluerdelamort'
        }
    })

    const mailOption = {
        from: 'lordarglax@gmail.com',
        to: email,
        subject: 'Sports and Melvin : Order Placed',
        html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
    ...
    db.collection('order').doc(docName).set(req.body)
        .then(data => {
            ...
            transporter.sendMail(mailOption, (err, info) => {
                if (err) {
                    res.json({ 'alert': 'opps! its seems like some error occured. Try again' })
                } else {
